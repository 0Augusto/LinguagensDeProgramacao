...
A linguagem de programação Go é multiparadigma, dentre elas o imperativo, funcional e concorrente. Ela possui loops, declarações e seleções, a linguagem foi projetada com suporte nativo para concorrência, o que significa que é fácil escrever programas que executam tarefas simultaneamente, aproveitando os recursos de hardware modernos, como múltiplos núcleos de processamento. Embora Go não seja uma linguagem de programação funcional pura, ela suporta muitos conceitos de programação funcional. As funções são cidadãos de primeira classe em Go, o que significa que podem ser passadas e manipuladas como outros tipos de dados e por fim Go tem suporte nativo para operações concorrentes. A concorrência em Go inclui multithreading, multiprocessamento e assincronia[et. al].
